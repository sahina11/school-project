...
/* The process pointed to by 'mp' has forked.  Create a child process. */
  ...
  *rmc = *rmp;			/* copy parent's process slot to child's */
   ...
   * LUCAS   *
   ***********/
  //increase ap_sema[]->refcount when forked, increase all in use semahpore's refcount (lucas)
  int l;
  for(l=0;l<SEMA_PER_SET;l++){
	  if(rmc->ap_sema[l] != NULL){
		  rmc->ap_sema[l]->refcount++;
	  }
  }
 ...
 */
  ...
  //decrement refcount of every semaphore in use for such process (lucas)
  int i;
  for(i=0;i<SEMA_PER_SET;i++){	//decrement every semaphore in use
	  if(rmp->ap_sema[i] != NULL){
		  rmp->ap_sema[i]->refcount--;

		  if(rmp->ap_sema[i]->refcount == 0){ //the such semaphore's refcount is 0, free memory and graft NULL on such sema ptr
			  free(rmp->ap_sema[i]);
			  rmp->ap_sema[i]= NULL;
		  }
	  }

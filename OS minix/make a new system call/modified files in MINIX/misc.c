 ...
 * =========*/
 ...
 int do_promote()
 {
	 int child_pid = m_in.m1_i1; //receive the msg from user program

	 int i; //for finding child process number
	 /* why do I need user program to pass me(pm) its child pid?
	  * ans: main is the one who get child pid right after fork, so he knows who is
	  * his new child.
	  *
	  * who should call the promote?
	  * ans:main()!,  instead of forked child process
	  * because main () is the one who know he is going to die, then promote his children
	  * child processes are busy doing things, moreover, it is illogical for children to
	  * "promote" themselves
	  *
	  * summary: mani() forked a child and got child pid, then main() is the one who
	  * should promote child, so main() call promote(child pid) in user layer, pass cpid
	  * to pm, pm picks up and do the real work( promotion)
	  */

	 for(i=0 ; i<= 255; i++){
		 if(mproc[i].mp_pid == child_pid){ //here we find child process number i
			 int main_proc_no = mproc[i].mp_parent; //find parent proc no. from child's mproc
			 int shell_proc_no = mproc[main_proc_no].mp_parent; //one step further, to find grand parents proc.no

			 mproc[i].mp_parent = shell_proc_no; //got it, assign, finished
